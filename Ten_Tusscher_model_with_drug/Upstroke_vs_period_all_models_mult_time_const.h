//
//  Upstroke_vs_period_all_models_mult_time_const.h
...
#ifndef Upstroke_vs_period_all_models_mult_time_const_h
#define Upstroke_vs_period_all_models_mult_time_const_h
...
void Upstroke_vs_period_all_models_mult_time_const(){
    ...
    int counter; //counts the number of time steps that have been taken
    ...
    chdir("../Ten_Tusscher_model_with_drug_output/Upstroke_vs_Period_nonVdepend_2_28_18/");// Non-V-dependent results //"/Users/z3525459/Library/CloudStorage/OneDrive-UNSW/Cardiac_research/Drug_interaction_paper_2017/JTB_2022_paper_C++_codes/Ten_Tusscher_model_with_drug_output/Upstroke_vs_Period_3_tau_b_12_5_17/"); //V-dependent results //
    
    std::ofstream result_file;

    
    double Diffusion_vec[3] = {1.0e5, 1.0e3, 1.0e1};  //(M^-1ms^-1) Different Diffusion values used for drug binding.
    ...
    for(Bind_Scheme = 0; Bind_Scheme < 9; Bind_Scheme++){//Looping through all binding schemes
        ...
        for (int Diff_counter = 0; Diff_counter < 3; Diff_counter++) { //Loops through the various drug binding rates
            ...
            if (Bind_Scheme == 0) {
                result_file.open(Model_type+'_'+bindscheme+'_'+"drug_Upstroke_v_Period_stimdur06.txt");
                
                Diff_counter = 3; //skips Diff_counter ahead so the no drug case is only run once (not for all 3 diffusion rates).
            } else {
                result_file.open(Model_type+'_'+bindscheme+'_'+"drug_Upstroke_v_Period_Diffval"+Diffval+"_kd10e-6_Drug20e-6_stimdur06.txt");
            }
                ...
                for (S1_cycle = 0; S1_cycle <=500; S1_cycle = S1_cycle + 1){                //Enter the number of beats that you want to simulate
                    ...
                    for (tTemp = 0; tTemp <= interval; tTemp=tTemp+dt){
                        ...
                        if (((interval - tTemp)<dt) && S1_cycle > 498) {
                            result_file << jj << ", " << S1_cycle << ", " << Cell.peak_slope << ", " << Cell.APD_90 << std::endl;//recording the BCL, number of stimuli, peak Upstroke, and APD90 for each stimuli
                        }
                        ...
                        if (fmod(t,5000)<dt) {std::cout << "S1_cycle=" <<S1_cycle<< ", " << "t=" << t << ", " "interval =" << interval << ", " << "Drug = "<< Drug << ", " << "Mutant = "<< mutant << std::endl;}//outputting progress to command line
...
#endif /* Upstroke_vs_period_all_models_mult_time_const_h */
