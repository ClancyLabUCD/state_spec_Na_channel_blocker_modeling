...
%7-4-16
%Code to compute steady-state b as a funciton of V for each drug model when
%drug binding rates are independent of V
...
b_infty_equal = @(D)(D*k_on./(D*k_on + k_off)); %steady state value of b

tau_b_equal = @(D)(1./(D*k_on + k_off)); %time constant of b

%for Gate immobilization model with binding to the inactive state
b_infty_gi_inact = @(V,D)(b_infty_equal(D).*(1-h_infty(V))./(1-b_infty_equal(D).*...
    h_infty(V))); %steady state value of b

tau_b_gi_inact = @(V,D)(tau_b_equal(D)./(1-h_infty(V).*b_infty_equal(D)));
...
%for Gate immobilization model with binding to the noninactivated states
b_infty_gi_noninact = @(V,D)(b_infty_equal(D).*(1-h_infty_bar(V))./(1-b_infty_equal(D).*...
    h_infty_bar(V)));

tau_b_gi_noninact = @(V,D)(tau_b_equal(D)./(1-h_infty_bar(V).*b_infty_equal(D)));
%time constant of drug binding (assuming h = h_infty)
...
figure(6)
p1 = plot(V_vec, b_infty_equal(Drug).*ones(1,length(V_vec)), '-',...
    V_vec, b_infty_equal(Drug).*ones(1,length(V_vec))+.01, '--',...
    V_vec, b_infty_gi_inact(V_vec, Drug), '-',...
    V_vec, b_infty_gi_noninact(V_vec, Drug), '--');
xlabel('V (mV)');
ylabel('b_\infty');
%legend('K_D', 'b_\infty Inactive', 'b_\infty Closed');
p1(1).Color = c{1};
p1(2).Color = c{1};
p1(3).Color = c{2};
p1(4).Color = c{2};
