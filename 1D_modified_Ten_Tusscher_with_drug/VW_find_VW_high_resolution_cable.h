//
//  VW_find_VW_high_resolution_cable.h
//  1D_modified_Ten_Tusscher_with_drug_2017_Summer
//
//  Created by Steffen Docken on 10/10/17.
...
#ifndef VW_find_VW_high_resolution_cable_h
#define VW_find_VW_high_resolution_cable_h
...
void VW_find_VW_high_resolution_cable(){
#ifdef use_omp
    ...
    //initialization
    double BCL;
    const double Sim_len = 1.0e3; //length of time simulated
    ...
    error_file.open(("errors_VW_find_VW_dt" + dt_val +"_dx"+dx_val+".txt").c_str());  //creates and opens the file to store any errors that occur when loading the initial conditions
    
    
    //Initilizations
    double t_S2, t_S2_start;
    const int V_num = (int) (Sim_len + 0.5)/2 + 1; //This is the number of time points at which Vs should saved
    ...
    for (int jj = 0; jj < 3; jj++) {  //Looping over the BCL values for which the VW will be checked.
        ...
        for(Bind_Scheme = 0; Bind_Scheme < 5; Bind_Scheme++){ //looping over the various Binding Schemes
            
            if (Bind_Scheme == 1) {
                Bind_Scheme = 2; //to Skip GRI in this program
            }
            ...
            //chdir ("../1D_modified_Ten_Tusscher_with_drug_output/VW_initialization_VW_initialization_12_15_17/"); //for my laptop (changes the directory from which the iniitial conditions will be loaded.  When this code is run on linux system, all initial condition files must be in the same folder as where this program is saved.)
            ...
            //chdir ("../1D_modified_Ten_Tusscher_with_drug_output/VW_results_12_22_17/"); //for my laptop (changes the directory from which the iniitial conditions will be loaded.  When this code is run on linux system, all initial condition files must be in the same folder as where this program is saved.)
            ...
            if (jj == 0) {
                if (Bind_Scheme == 0) {
                    t_S2_start = 354.5;
                } else if(Bind_Scheme == 2){
                    t_S2_start = 362.0;
                } else if(Bind_Scheme == 3){
                    t_S2_start = 357.0;
                } else if(Bind_Scheme == 4){
                    t_S2_start = 361.0;
                }
            } else if (jj == 1){
                if (Bind_Scheme == 0) {
            ...
            } //This gets initial test for S2 close to the VW (based on low resolution results)
            
            for (t_S2 = t_S2_start; t_S2 < 420.0; t_S2 = t_S2 + 0.05) {
                flag_retro_prop1 = 0;
...
#pragma omp parallel for
                    ...
                    if (fmod(C_ptr->tTemp + dt/2,2)<dt) { //adding dt/2 to allow tTemp to be between dt/2 less than an even number and dt/2 more than an even number
                ...
                } //end of for loop for time before the S2 stimulus
...
#pragma omp parallel for
                    ...
                    if (fmod(C_ptr->tTemp + dt/2 + t_S2,2)<dt) { //adding dt/2 to allow tTemp +t_S2 to be between dt/2 less than an even number and dt/2 more than an even number
                        prev_V_mat[mat_index][0] = V_mat[mat_index][0];
                ...
                } //end of for loop for time after S2 stimulus
                ...
                if (flag_retro_prop2 == 1) {
                    ...
                    for (int ii = 0; ii < (int) (Sim_len + 0.5)/2 + 1; ii ++){
                    ...
                    } //This gets initial test for S2 close to the end of VW (based on low resolution results)
                    
                    t_S2 = t_S2 - 0.05; //This is to compensate for the fact that 0.05 will be added to t_S2 at the end of this loop.  Therefore, in the next loop, t_S2 will be the value assigned in the previous if-else statement.
            ...
            }//End of for loop through t_S2
            
            result_file_S2_times.close(); //closes the file saving the times of the VW borders
            
            
        }// end of loop through bind_scheme
        
        
    }//end of loop through BCL
    
    result_file_time.open(("VW_find_VW_dt" + dt_val +"_dx"+dx_val+"_timing.txt").c_str());
...
#endif /* VW_find_VW_high_resolution_cable_h */
