...
%formulation of the guarded receptor model with the ten Tusscher model for
...
function [h_ss,b_ss] = HH_b_star(A, DI, V_A, V_DI, k_on, k_off_0, Drug,...
...
k_off = @(V)(k_off_0*exp(-0.7*V*F/(R*T))); %unbinding rate
...
elseif (GIHH_GRHH_nons == 1) %guarded receptor model (HH formulation)
    if (inact_noninact == 0) %inactive state
        ...
        b_exp_A_inact = exp(-1/tau_b_GRHH(V_A,Drug)*((1-h_infty_GRHH(V_A))*A + ...
            (h_infty_GRHH(V_A)-h_ss)*tau_h_GRHH(V_A)*...
            (1-exp(-A/tau_h_GRHH(V_A)))));

        b_exp_DI_inact = exp(-1/tau_b_GRHH(V_DI,Drug)*((1-h_infty_GRHH(V_DI))*DI +...
            (h_infty_GRHH(V_DI) - h_infty_GRHH(V_A) +(h_infty_GRHH(V_A) - h_ss)*...
            exp(-A/tau_h_GRHH(V_A)))*tau_h_GRHH(V_DI)*(1-exp(-DI/tau_h_GRHH(V_DI)))));

        b_ss = (b_infty_GRHH(V_DI,Drug) - (b_infty_GRHH(V_DI,Drug)-b_infty_GRHH(V_A,Drug)*...
            (1-b_exp_A_inact))*b_exp_DI_inact)/...
            (1-b_exp_A_inact*b_exp_DI_inact);
        
    elseif (inact_noninact == 1) %non-inactive state block
         b_exp_A_noninact = exp(-1/tau_b_GRHH(V_A,Drug)*(h_infty_GRHH(V_A)*A -...
             (h_infty_GRHH(V_A) - h_ss)*tau_h_GRHH(V_A)*...
             (1-exp(-A/tau_h_GRHH(V_A)))));
         
         b_exp_DI_noninact = exp(-1/tau_b_GRHH(V_DI,Drug)*(h_infty_GRHH(V_DI)*...
             DI - (h_infty_GRHH(V_DI) - h_infty_GRHH(V_A) + (h_infty_GRHH(V_A) - ...
             h_ss)*exp(-A/tau_h_GRHH(V_A)))*tau_h_GRHH(V_DI)*(1 - ...
             exp(-DI/tau_h_GRHH(V_DI)))));
         
         b_ss = (b_infty_GRHH(V_DI, Drug) - (b_infty_GRHH(V_DI, Drug) - ...
             b_infty_GRHH(V_A, Drug)*(1 - b_exp_A_noninact))*b_exp_DI_noninact)/...
             (1 - b_exp_A_noninact*b_exp_DI_noninact);   
    end
    
elseif (GIHH_GRHH_nons == 2) %non-state-specific binding
    b_ss = (b_infty_GRHH(V_DI,Drug) - (b_infty_GRHH(V_DI,Drug) - (1 -...
        exp(-A/tau_b_GRHH(V_A,Drug)))*b_infty_GRHH(V_A,Drug))*exp(-DI/...
        tau_b_GRHH(V_DI,Drug)))/(1 - exp(-A/tau_b_GRHH(V_A,Drug) - ...
        DI/tau_b_GRHH(V_DI,Drug)));
